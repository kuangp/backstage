  ...
  implements EntityProvider, EventSubscriber
{
  ...
  async onEvent(params: EventParams): Promise<void> {
    ...
    if (!orgs.includes(eventPayload.organization.login)) {
      return;
    }
    ...
    if (params.topic.includes('team')) {
      if (
        ...
        await this.onTeamChangeInOrganization(eventPayload as TeamEvent);
  ...
  private async onMemberChangeInOrganization(
    ...
    if (event.action === 'member_removed') {
      if (userApplicableOrgs.length) {
        ...
        createDeltaOperation = this.createAddEntitiesOperation;
        ...
        createDeltaOperation = this.createRemoveEntitiesOperation;
  ...
  private async onTeamChangeInOrganization(event: TeamEvent) {
    if (!this.connection) {
      throw new Error('Not initialized');
    }
    ...
    const createDeltaOperation =
      event.action === 'created'
        ? this.createAddEntitiesOperation
        : this.createRemoveEntitiesOperation;
