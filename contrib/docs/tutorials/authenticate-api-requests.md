...
import cookieParser from 'cookie-parser';
import { authMiddlewareFactory } from './utils';

  export default rootHttpRouterServiceFactory({
~_  configure: ({ app, config, middleware, routes }) => {
...
~_    app.use(authMiddlewareFactory({ config }));
